			...
			if ((str.find_first_not_of(" \t\r\n") != string::npos) && (str[0] != '#')) {
				...
				// Note that no transforms/stacks are applied to the colors. 
					...
					if (validinput) {
						...
						eyeinit = glm::vec3(lookfromx, lookfromy, lookfromz);
						upinit = glm::vec3(upx, upy, upz);
						upinit = glm::normalize(upinit);
						center = glm::vec3(lookatx, lookaty, lookatz);
						// TODO: how to use upvector fn in Transform.cpp ? 
						Transform::upvector(upinit, center);
				...
				// Also look at demo.txt to get a sense of why things are done this way.
					...
					if (validinput) {
						...
						vec3 v = glm::vec3(values[0], values[1], values[2]);
						glm::translate(mat4(1.0f), v);
						mat4 model = transfstack.top();
						mat4 translation = model * glm::translate(mat4(1.0f), v);
						transfstack.pop();
						transfstack.push(translation);
					}
				}
				else if (cmd == "scale") {
					validinput = readvals(s, 3, values);
					if (validinput) {
						...
						mat4 model = transfstack.top();
						mat4 transform = model * glm::scale(glm::mat4(1.0f), v);
						transfstack.pop();
						transfstack.push(transform);
					}
				}
				else if (cmd == "rotate") {
					validinput = readvals(s, 4, values);
					if (validinput) {
						...
						glm::mat3 rotation_matrix = Transform::rotate(angle, axis);
						glm::mat4 rotation = mat4(rotation_matrix);
						mat4 model = transfstack.top();
						mat4 transform = model * rotation;
						transfstack.pop();
						transfstack.push(transform);
					}
				}
